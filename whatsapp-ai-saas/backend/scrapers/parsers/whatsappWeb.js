/**
 * Parseurs DOM WhatsApp Web, extraits des callbacks injectés dans orderListener.js
 * (poller liste / MutationObserver) et WorkArea.jsx (getWhatsAppContext).
 *
 * Chaque fonction est AUTO-PORTANTE : elle ne référence que `document` (ou l'arg
 * `doc`/`el`/`msg` passé) et des helpers locaux inlinés — jamais la portée du
 * module. Playwright/Puppeteer peut donc sérialiser son code source pour
 * `page.evaluate(fn)`, et le webview Electron via `executeJavaScript` +
 * `fn.toString()`. Les tests unitaires appellent ces fonctions sur un document
 * jsdom.
 */
