...
# Mesure de couverture (rapport console + lcov dans coverage/, sans seuil)
npm run test:coverage

# Smoke E2E — vraie application Electron (prérequis : npm install SANS
# --ignore-scripts, puis npm run build) ; sous Linux sans affichage :
# xvfb-run -a npm run test:e2e  — exécuté automatiquement par la CI (job e2e-smoke)
npm run test:e2e
...
**Couverture mesurée en CI depuis v1.48.1** (constat C5 levé) : la CI exécute `vitest run --coverage` (rapport texte dans les logs + artifact `lcov.info`), **sans seuil bloquant** — la mesure sert à piloter les contributions, pas à les bloquer. Base mesurée au jour de la publication : ~13 % global, avec des modules cœur déjà solides (`db.js` 81 %, `logRedact.js` 100 %, `phoneFormat.js` 100 %, `llmJson.js` 87 %). Le smoke E2E Playwright (démarrage réel `Electron → main.cjs → backend Express` sous xvfb) est branché via le job `e2e-smoke`, fourni prêt dans `e2e/ci-snippet.yml` — sa publication dans le workflow exige la permission `workflows` (constat H2), réservée à l'équipe éditrice.

**État réel de la couverture (v1.48.4).** **262 tests** répartis dans **28 suites** couvrent aujourd'hui les zones qui cassent en production : chiffrement des secrets (`secretStore`, 15 tests — dont l'observabilité de l'échec de déchiffrement et la trace de régénération de clé), extraction JSON des réponses LLM (`llmJson`, 18), numérotation (`phoneRules` 20 + `phoneFormat` 13), migrations SQLite (5), parité i18n des 4 langues (3), CLI entrant (`cliInbound`, 13) et flux CLI/MCP bout-en-bout (`cliMcpFlow`, 6), routage agentique (`agentFallback`, 6 — les tests d'intégration réels s'ignorent proprement sans clé API ; `agentFallbackStrategies`, 6 — mécanique des stratégies CLI/API/MCP et repli, remontée du stderr d'échec et trust workspace Gemini, déterministe et sans clé), pré-vol d'authentification Gemini CLI (`externalAgentRunner`, 5), CRM (`crmService`, 6), parseurs de scrapers (14), comparaison de versions de l'updater (14), etc. Le gros du code reste non testé, et c'est la contribution la plus utile qu'on puisse apporter au projet. Les zones encore à couvrir :
